package com.capgemini.kafka.general.service.impl;
...
public class LoginController {
  ...
  {
	  System.out.println("Kafka Send Message called");
	  try {
		  KafkaProducerLogic logic = new KafkaProducerLogic();

		  System.out.print("Producer Logic constructor" + logic);


		  logic.sendMessage("test", "This is devon test");

	  }
	  catch(Exception ex) {
		  System.out.print("This is done" + ex.getMessage());
	  }
	  ModelAndView model = new ModelAndView();
	  model.addObject("msg", "message sent successful!");
	  return model;
